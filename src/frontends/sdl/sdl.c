	...
	case FB_INSERT_FILE_INTO_PL:
		...
		else
		{
			...
			if (sel_file)
			{
				...
				if (path)
				{
					if (user_key_action == FB_INSERT_FILE_INTO_PL)
					{ /* insert item */
...
static void m_draw(M *m, SDL_Surface *t)
{
	...
	if (m->so < 3 && m->st < 3)
	{
		...
		if (skin.arrow_up)
			SDL_BlitSurface(skin.arrow_up, &srect, t, &drect);
		if (skin.arrow_down)
		{
			int i;
		...
		if (dx > 0)
		{
			if (dy > 0 && y > cy && cy + 6 < gmu_widget_get_height((GmuWidget *)&skin.lv, 1) - 3 * srect.h)
				cy += 6;
			if (dy < 0 && y < cy && cy - 6 > 0)
				cy -= 6;
		}
		...
		if (y > gmu_widget_get_height((GmuWidget *)&skin.lv, 1) - skin.arrow_down->h - s)
			dy = -s;
	...
	{
		if (m->so > m->st)
			snprintf(tmp, 63, "%c %c %c", 53, 61, 66);
		else
			snprintf(tmp, 63, "%c%cM%c%c%cV%cR", 71, 65, 69, 32, 48, 69);
		textrenderer_draw_string_with_highlight(&skin.font1, &skin.font2, tmp, 0, t,
												(gmu_widget_get_width((GmuWidget *)&skin.lv, 1) -
												 textrenderer_get_string_length(tmp) * skin.font1.chwidth) >>
													1,
...
static void run_player(char *skin_name, char *decoders_str)
{
	...
	if (quit == DONT_QUIT)
	{
		...
		cover_viewer_init(&cv, &skin,
						  strncmp(cfg_get_key_value(*config, "CoverArtworkLarge"), "yes", 3) == 0 ? 1 : 0,
						  strncmp(cfg_get_key_value(*config, "SmallCoverArtworkAlignment"), "left", 4) == 0 ? ALIGN_LEFT : ALIGN_RIGHT,
						  strncmp(cfg_get_key_value(*config, "LoadEmbeddedCoverArtwork"), "first", 5) == 0 ? EMBEDDED_COVER_FIRST : (strncmp(cfg_get_key_value(*config, "LoadEmbeddedCoverArtwork"), "last", 4) == 0 ? EMBEDDED_COVER_LAST : EMBEDDED_COVER_NO));
		...
		seconds_until_backlight_poweroff = (seconds_until_backlight_poweroff > 0 ? seconds_until_backlight_poweroff : -1);
	...
	{
		wdprintf(V_WARNING, "sdl_frontend", quit == QUIT_WITHOUT_ERROR ? "Strange. Exit was requested, shutting down." : "An error was detected, shutting down.");
	}
	...
	while (quit == DONT_QUIT && SDL_WaitEvent(&event))
	{
			...
			(button_repeat_timer == 0 && user_key_action > 0))
		{
			...
			if (!hold_state)
			{
				...
				if (amethod == ACTIVATE_PRESS)
					view = m_enable(&m, button, view);
		...
		if (update != UPDATE_NONE && update_display && !display_inactive)
		{
			...
			if ((update & UPDATE_DISPLAY) && frame_skip_counter == 0)
			{
				...
				player_display_draw(&skin.font_display, ti,
									(file_player_get_item_status() == STOPPED ? STOPPED : (gmu_core_playback_is_paused() ? PAUSED : PLAYING)),
			...
			if (update & UPDATE_TEXTAREA)
			{
				...
				case HELP:
					...
					break;
				case TRACK_INFO:
					if (file_player_is_metadata_loaded())
	...
	if (quit != QUIT_WITH_ERROR)
	{
		if (strncmp(cfg_get_key_value(*config, "RememberSettings"), "yes", 3) == 0)
		{
			...
			cfg_add_key(config, "TimeDisplay", time_remaining ? "remaining" : "elapsed");
			if (buffer)
			{
...
static void *start_player(void *arg)
{
	...
	if (start || setup)
	{
		if (skin_name[0] == '\0')
		{
			...
			if (skinname)
				strncpy(skin_name, skinname, 127);
			skin_name[127] = '\0';
		}
	}
	/*if (setup) {
